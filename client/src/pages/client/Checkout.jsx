...
const Checkout = () => {
  ...
  const discountPercent = product.discountPercent || Math.round(
    ((originalUnitPrice - discountedUnitPrice) / originalUnitPrice) * 100
  );
  ...
  const handleReviewSubmit = async (e) => {
    ...
    if (comment.length > 500) {
      setReviewError("Review cannot be more than 500 characters.");
      return;
    }
  ...
  const validate = () => {
    ...
    if (!/^01\d{9}$/.test(formdata.phone.trim())) {
      setError({ field: "phone", message: "Please enter a valid 11-digit phone number." });
      return false;
    }
    ...
    if (formdata.paymentMethod === "bkash" || formdata.paymentMethod === "nagad") {
      ...
      if (!/^01\d{9}$/.test(formdata.mobileNumber.trim())) {
  ...
  const submitHandler = async (e) => {
    ...
    if (!accessToken) {
      setServerMessage("Please login before placing an order.");
    ...
    try {
      ...
      setServerMessage(error.response?.data?.message || "Failed to place order. Please try again.");
      setServerMessageType("error");
    } finally {
      setLoading(false);
    }
  ...
  return (
    <div className="min-h-screen mt-[45px] sm:mt-[50px] py-4 sm:py-8 px-3 sm:px-4 md:px-8">
      ...
      <div className="max-w-6xl mx-auto w-full flex flex-row items-center justify-between mb-3 sm:pb-4 gap-2">
        ...
        <button
          onClick={() => navigate(-1)}
          className="text-sm md:text-base text-[#B08946] hover:underline font-medium flex items-center gap-1.5"
        >
          <FontAwesomeIcon icon={faArrowLeft} size="sm" />
          Back to Product
        </button>
      ...
      <div className="max-w-6xl mx-auto w-full grid grid-cols-1 lg:grid-cols-12 gap-5 sm:gap-8 items-start">
        ...
        <div className="lg:col-span-5 flex flex-col gap-4 sm:gap-5">
          ...
          <div className="bg-white rounded-2xl border border-[#E4DDCE] p-3.5 sm:p-4 shadow-sm hover:shadow-md transition">
            ...
            <div className="relative bg-[#FAF6EF]/60 rounded-xl border border-[#E4DDCE]/60 overflow-hidden p-3 sm:p-4 flex justify-center items-center h-56 sm:h-72 mb-3 sm:mb-4 group">
              ...
              <div className="absolute top-2.5 left-2.5 sm:top-3 sm:left-3 flex flex-col gap-1 items-start">
                <span className="bg-[#B08946] text-white text-xs sm:text-sm uppercase tracking-wider font-semibold px-2 sm:px-2.5 py-0.5 rounded-full shadow-sm">
                  In Stock
                </span>
                {product.stock < 10 && product.stock > 0 && (
                  <span className="bg-amber-500 text-white text-xs sm:text-sm uppercase tracking-wider font-semibold px-2 sm:px-2.5 py-0.5 rounded-full shadow-sm">
                    Only {product.stock} Left
                  </span>
                )}
              </div>
              {discountPercent > 0 && (
                <div className="absolute top-2.5 right-2.5 sm:top-3 sm:right-3 bg-red-600 text-white text-xs sm:text-sm font-extrabold px-2 sm:px-2.5 py-0.5 sm:py-1 rounded-lg shadow-md flex items-center gap-1">
            ...
            {productImages.length > 0 && (
              <div className="mb-4">
                <p className="text-xs sm:text-sm font-medium text-[#16241F]/60 mb-1.5 uppercase tracking-wider">
                  Select Image View
                </p>
            ...
            <div className="space-y-2.5 sm:space-y-3 pt-3 border-t border-[#E4DDCE]/60">
              <div className="flex justify-between items-start gap-2">
                <div>
                  <h2 className="text-lg sm:text-xl font-bold text-[#16241F] leading-snug">
                    {product.name}
                  </h2>
                  {product._id && (
                    <span className="text-xs sm:text-sm text-[#16241F]/50">
                ...
                <div className="text-right">
                  <div className="flex items-center gap-1.5 sm:gap-2 justify-end">
                    {originalUnitPrice > discountedUnitPrice && (
                      <span className="text-sm sm:text-base text-[#16241F]/40 line-through font-medium">
                        ৳{originalUnitPrice * count}
                      </span>
                    )}
                    <span className="text-xl sm:text-2xl font-extrabold text-[#16241F]">
                      ৳{discountedUnitPrice * count}
                    </span>
                  </div>
                  {discountPercent > 0 && (
                    <span className="text-xs sm:text-sm text-green-600 font-semibold block">
                      You Save ৳{(originalUnitPrice - discountedUnitPrice) * count}
                    </span>
                  )}
                </div>
              </div>
              {product.description && (
                <p className="text-sm text-[#16241F]/70 leading-relaxed line-clamp-2">
                  {product.description}
                </p>
              )}
              {/* QUANTITY */}
              <div className="pt-1.5 sm:pt-2">
                <div className="flex justify-between items-center py-1.5 sm:py-2 px-2.5 sm:px-3 bg-[#FAF6EF] rounded-xl border border-[#E4DDCE]">
                  <span className="text-xs sm:text-sm font-semibold uppercase text-[#16241F]/80 tracking-wider">
                    Quantity
                  </span>
                  <div className="flex items-center bg-white border border-[#E4DDCE] rounded-lg overflow-hidden shadow-sm">
                    <button
                      ...
                      className="px-2.5 sm:px-3 py-1.5 text-[#16241F] hover:bg-[#FAF6EF] transition"
                    >
                      <FontAwesomeIcon icon={faMinus} size="sm" />
                    </button>
                    <span className="px-3 sm:px-4 font-bold text-sm sm:text-base text-[#16241F]">
                      {count}
                    </span>
                    <button
                      type="button"
                      onClick={() => setCount((prev) => (prev < (product.stock || 100) ? prev + 1 : prev))}
                      className="px-2.5 sm:px-3 py-1.5 text-[#16241F] hover:bg-[#FAF6EF] transition"
                    >
          ...
          <div className="bg-white rounded-2xl border border-[#E4DDCE] p-4 sm:p-5 shadow-sm space-y-4">
            <h3 className="text-sm sm:text-base font-bold uppercase tracking-wider text-[#16241F]/80 pb-2 border-b border-[#E4DDCE]">
              Ratings & Reviews
            </h3>
            <form onSubmit={handleReviewSubmit} className="space-y-3 bg-[#FAF6EF]/50 p-3 rounded-xl border border-[#E4DDCE]/60">
              <div>
                <label className="block text-sm font-semibold text-[#16241F]/80 mb-1">
                  Your Rating
                </label>
                <div className="flex items-center gap-1">
                  {[1, 2, 3, 4, 5].map((star) => (
                    ...
                    >
                      <FontAwesomeIcon
                        icon={star <= userRating ? faStar : faStarOutline}
                        className={star <= userRating ? "text-amber-500 text-lg sm:text-xl" : "text-gray-300 text-lg sm:text-xl"}
                      />
                    </button>
                  ))}
                  <span className="text-sm text-[#16241F]/60 ml-2 font-medium">
                    {userRating}/5
                  </span>
                </div>
              </div>
              <div>
                <textarea
                  ...
                  className="w-full border border-[#E4DDCE] rounded-lg p-2.5 text-sm outline-none focus:border-[#B08946] bg-white resize-none"
                />
              ...
              <div className="flex items-center justify-between">
                <button
                  type="submit"
                  disabled={reviewLoading || !reviewComment.trim() || userRating === 0}
                  className="bg-[#B08946] hover:bg-[#967339] text-white text-sm font-semibold px-3.5 py-2 rounded-lg flex items-center gap-1.5 disabled:opacity-50 transition"
                >
                  <FontAwesomeIcon
                    icon={reviewLoading ? faSpinner : faPaperPlane}
                ...
                {reviewSubmitted && (
                  <span className="text-sm text-green-600 font-medium">Review added!</span>
                )}
              </div>
            </form>
            <div className="space-y-2.5 max-h-64 overflow-y-auto pr-1">
              {reviewsList.length === 0 ? (
                <p className="text-sm text-[#16241F]/50 text-center py-2 italic">
                  No reviews yet. Be the first to review this product!
                </p>
              ) : (
                reviewsList.map((review) => (
                  <div key={review._id} className="p-2.5 bg-[#FAF6EF]/30 rounded-lg border border-[#E4DDCE]/40 space-y-1">
                    <div className="flex items-center justify-between">
                      <div className="flex gap-0.5">
                        {[1, 2, 3, 4, 5].map((star) => (
                          <FontAwesomeIcon
                            key={star}
                            icon={star <= review.rating ? faStar : faStarOutline}
                            className={star <= review.rating ? "text-amber-500 text-sm" : "text-gray-300 text-sm"}
                          />
                        ))}
                      </div>
                    ...
                    <p className="text-sm font-semibold text-[#16241F]">
                      {review.user?.fullname || review.user?.name || "Customer"}
                    </p>
                    <p className="text-sm text-[#16241F]/80">{review.comment}</p>
                  </div>
                ))
              )}
            </div>
        ...
        <div className="lg:col-span-7 flex flex-col gap-5">
          ...
          <div className="bg-white rounded-2xl border border-[#E4DDCE] shadow-sm p-3.5 sm:p-6 md:p-8">
            <div className="flex items-center gap-2 mb-3.5 sm:mb-6 pb-2.5 sm:pb-3 border-b border-[#E4DDCE]">
              <FontAwesomeIcon icon={faShoppingBag} className="text-[#B08946]" />
              <h3 className="text-lg sm:text-xl font-bold text-[#16241F]">
            ...
            <form className="space-y-3.5 sm:space-y-5" onSubmit={submitHandler} id="checkout-form">
              ...
              <div>
                ...
                <input
                  ...
                  className={`w-full border rounded-xl px-3.5 sm:px-4 py-2 sm:py-2.5 text-sm sm:text-base outline-none transition ${error.field === "name"
              ...
              <div>
                ...
                <div className="relative">
                  <span className="absolute left-3.5 sm:left-4 top-3/4 -translate-y-1/2 text-[#16241F]/50 text-sm sm:text-base font-medium">
                    +880
                  </span>
                  <input
                    type="tel"
                    ...
                    maxLength={11}
                    className={`w-full border rounded-xl pl-14 sm:pl-16 pr-3.5 sm:pr-4 py-2 sm:py-2.5 text-sm sm:text-base outline-none transition ${error.field === "phone"
                      ? "border-red-500 bg-red-50/20 ring-1 ring-red-500"
                      : "border-[#E4DDCE] bg-[#FAF6EF]/30 focus:border-[#B08946] focus:ring-2 focus:ring-[#B08946]/20"
                      }`}
                  />
              ...
              <div>
                ...
                <textarea
                  ...
                  className={`w-full border rounded-xl px-3.5 sm:px-4 py-2 sm:py-2.5 text-sm sm:text-base outline-none resize-none transition ${error.field === "address"
                    ? "border-red-500 bg-red-50/20 ring-1 ring-red-500"
                    : "border-[#E4DDCE] bg-[#FAF6EF]/30 focus:border-[#B08946] focus:ring-2 focus:ring-[#B08946]/20"
                    }`}
                />
                      ...
                      >
                        <div className="flex items-center gap-2.5 sm:gap-3">
                          <input
                            ...
                            onChange={changeHandler}
                            className="accent-[#B08946] w-4 h-4 sm:w-4.5 sm:h-4.5"
                          />
                          <div className="flex items-center gap-1.5 sm:gap-2">
                            <FontAwesomeIcon
                              icon={method === "bkash" || method === "nagad" ? faMobileAlt : faStore}
                              className={
                                method === "bkash"
                                  ? "text-pink-600"
                                  : method === "nagad"
                            ...
                            />
                            <span className="font-semibold text-sm sm:text-base text-[#16241F] capitalize">
                              {method === "cod" ? "Cash on Delivery (COD)" : method}
                            </span>
                          </div>
                        </div>
                        {formdata.paymentMethod === method && (
                          <FontAwesomeIcon icon={faCircleCheck} className="text-[#B08946]" />
                        )}
                      </div>
                      {(method === "bkash" || method === "nagad") && formdata.paymentMethod === method && (
                        <div className="mt-2 sm:mt-2.5 pl-2 sm:pl-4 pr-1">
                          <div className="p-3 sm:p-4 bg-[#FAF6EF] rounded-xl border border-[#E4DDCE] space-y-2.5 sm:space-y-3">
                            <div>
                              <label className="block text-xs sm:text-sm font-medium text-[#16241F]/80 mb-1">
                              ...
                              <input
                                ...
                                placeholder="01XXXXXXXXX"
                                maxLength={11}
                                className={`w-full border rounded-lg px-3 py-1.5 sm:py-2 text-sm sm:text-base outline-none transition ${error.field === "mobileNumber"
                                  ? "border-red-500"
                            ...
                            <div>
                              ...
                              <input
                                ...
                                className={`w-full border rounded-lg px-3 py-1.5 sm:py-2 text-sm sm:text-base outline-none transition ${error.field === "transactionId"
                                  ? "border-red-500"
                                  : "border-[#E4DDCE] bg-white focus:border-[#B08946]"
                                  }`}
                              />
          ...
          <div className="bg-white rounded-2xl border border-[#E4DDCE] p-4 sm:p-5 shadow-sm space-y-2.5 sm:space-y-3">
            ...
            <div className="space-y-1.5 sm:space-y-2 text-sm sm:text-base">
              <div className="flex justify-between text-[#16241F]/70">
                <span>Original Price ({count} {count > 1 ? "items" : "item"})</span>
                <span className="font-medium text-[#16241F]/60 line-through">৳{totalOriginalSubtotal}</span>
              </div>
              {discountAmount > 0 && (
            ...
            <div className="border-t border-[#E4DDCE] pt-2.5 sm:pt-3 flex justify-between items-baseline">
              <span className="font-bold text-sm sm:text-base text-[#16241F]">Total Amount</span>
              <span className="text-2xl sm:text-3xl font-black text-[#16241F]">৳{finalTotal}</span>
            </div>
            <button
              type="submit"
              form="checkout-form"
              disabled={loading || !accessToken}
              className="w-full bg-[#16241F] hover:bg-[#0F1A16] text-[#FAF6EF] py-3 sm:py-3.5 rounded-xl font-bold text-sm sm:text-base flex items-center justify-center gap-2 disabled:opacity-60 transition duration-200 mt-4 shadow-md active:scale-[0.99]"
            >
              <FontAwesomeIcon icon={loading ? faSpinner : faCircleCheck} className={loading ? "animate-spin" : ""} />
              {!accessToken
                ? "Please Login First"
                : loading
                  ? "Placing Order..."
